...
const SORT_OPTIONS = [
  ...
  { value: "rating_high", label: "Top Rated", icon: Sparkles },
...
const SECTION_TABS = [
  ...
  { key: "new", label: "New Leads", icon: Clock },
];

const TEMPERATURE_OPTIONS = [
  { value: "", label: "All", emoji: null },
  { value: "Hot", label: "Hot", emoji: "ðŸ”¥" },
  { value: "Warm", label: "Warm", emoji: "âš¡" },
...
export default function LeadsFeedPage() {
  ...
  const [activeSection, setActiveSection] = useState<"for-you" | "trending" | "new">("for-you");
  const [showAdvancedFilters, setShowAdvancedFilters] = useState(false);

  // â”€â”€â”€ DEBOUNCED SEARCH (500ms) â”€â”€â”€
  useEffect(() => {
  ...
  const filters = useMemo(
    () => ({ niche, location, temperature }),
    [niche, location, temperature]
  );

  const hasActiveFilters = niche || location || temperature;
  ...
  const {
    ...
    queryKey: ["leads-feed", debouncedSearch, sort, filters, activeSection],
    queryFn: async ({ pageParam = 1 }) => {
      ...
      if (activeSection === "trending") params.set("min_rating", "3.5");
      if (activeSection === "new") params.set("date_filter", "week");

      const res = await fetch(`/api/leads-feed?${params.toString()}`);
      if (!res.ok) {
        if (res.status === 401) throw new Error("Please login to view leads feed");
  ...
  // Subtle search indicator (not full skeleton)
  ...
  return (
    <div className="relative w-full min-h-screen bg-background ">
      ...
      {/* â”€â”€â”€ SUBTLE TOP LOADING BAR (Real-time search indicator) â”€â”€â”€ */}
      <AnimatePresence>
        {isSearching && (
          <motion.div
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6 ">
        ...
        {/* â”€â”€â”€ HERO SECTION (Apple Spotlight Style) â”€â”€â”€ */}
        ...
        >
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-sm sm:text-base text-muted-foreground max-w-xl mx-auto">
              Discover community-sourced business leads, updated in real-time.
            </p>
          </div>
          ...
          <div className="relative max-w-2xl mx-auto group">
            ...
            <Input
              placeholder="Search businesses, niches, locations..."
              value={searchInput}
              ...
              {SECTION_TABS.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex items-center gap-2 px-4 sm:px-5 py-2.5 text-xs sm:text-sm font-semibold rounded-lg transition-all duration-200",
                      isActive
        ...
        {/* â”€â”€â”€ FILTERS BAR (Inline Chips) â”€â”€â”€ */}
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
              ...
              >
                <div className="flex flex-col sm:flex-row gap-3 max-w-2xl mx-auto pt-2">
                  <div className="relative flex-1">
                    ...
                    <Input
                      placeholder="Niche (e.g., SaaS, Healthcare)"
                      value={niche}
                      onChange={(e) => setNiche(e.target.value)}
                      className="pl-10 h-10 bg-muted/30 border-border/50 focus-visible:ring-primary/30 rounded-lg text-sm"
                    />
                  </div>
                  <div className="relative flex-1">
                    <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground h-4 w-4" />
                    <Input
                      placeholder="Location (e.g., Lahore, NYC)"
                      value={location}
        ...
        {!isLoading && uniqueLeads.length > 0 && (
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Badge className="bg-primary/10 text-primary border-primary/20 text-xs px-2.5 py-1">
                {uniqueLeads.length} leads
              </Badge>
              <span className="text-xs text-muted-foreground hidden sm:inline">
                {activeSection === "for-you" && "Personalized for you"}
                {activeSection === "trending" && "Trending this week"}
                {activeSection === "new" && "Added recently"}
              </span>
            </div>
            {isSearching && (
        ...
        {uniqueLeads.length === 0 ? (
          ...
          >
            ...
            <h2 className="text-xl font-semibold text-foreground">No leads found</h2>
            <p className="text-sm text-muted-foreground max-w-sm">
              Try adjusting your filters, changing the section, or searching for different keywords.
            </p>
            <Button
              variant="outline"
              size="sm"
          ...
          >
            <AnimatePresence mode="popLayout">
              {uniqueLeads.map((lead, index) => {
                const isLast = index === uniqueLeads.length - 1;
                return (
                  <motion.div
                    key={lead.id}
                    ref={isLast ? lastLeadRef : undefined}
                    layout
                    initial={{ opacity: 0, scale: 0.95 }}
                    animate={{ opacity: 1, scale: 1 }}
        ...
        {/* â”€â”€â”€ INFINITE SCROLL INDICATORS â”€â”€â”€ */}
        {isFetchingNextPage && (
        ...
        {!hasNextPage && uniqueLeads.length > 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
...
// â”€â”€â”€ PREMIUM SKELETON (Only for initial load) â”€â”€â”€
